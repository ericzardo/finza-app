  ...
  export function ImportTransactionDialog({
    ...
    const handleConfirmImport = async () => {
      ...
      try {
        ...
        // LÓGICA DE SALDO INICIAL (Automático ou Manual)
        let openingBalanceToInject = calculatedOpeningBalance;

        // Se não achou no CSV, mas o usuário digitou o saldo ATUAL
        if (openingBalanceToInject === null && manualCurrentBalance !== null) {
            // Engenharia Reversa: Saldo Inicial = Saldo Final (Hoje) - Soma das Transações
            const totalTransactionsSum = parsedData.reduce((acc, curr) => acc + curr.amount, 0);
            
            // Ex: Tenho 1000 hoje. Gastei 200 no CSV (soma -200).
            // Inicial = 1000 - (-200) = 1200.
            openingBalanceToInject = manualCurrentBalance - totalTransactionsSum;
        }

        // Se temos um saldo inicial calculado (seja via CSV ou Manual)
        if (openingBalanceToInject !== null) {
          // Ajuste fino para arredondamento
          openingBalanceToInject = Math.round(openingBalanceToInject * 100) / 100;

          // Só injeta se for relevante (maior que 1 centavo ou se for zero absoluto explícito)
          // Aqui permitimos injetar transação de ajuste mesmo que pequena para bater o saldo
          const type = openingBalanceToInject >= 0 ? "INCOME" : "EXPENSE";
          ...
          transactionsToSend.push({
            ...
            amount: Math.abs(openingBalanceToInject),
          ...
          <div className="flex-1 overflow-hidden">
            {parsedData.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                {parsedData.length > 0 && calculatedOpeningBalance === null && (
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-amber-600 font-semibold text-sm">
                          Não localizamos o seu saldo atual no arquivo
                        </div>
                        <p className="text-xs text-muted-foreground max-w-md">
                          Para que seu extrato fique correto, informe quanto você tem nesta conta <strong>hoje</strong> (após essas transações).
                        </p>
                    </div>
                    
                    <div className="flex flex-col gap-1 min-w-37.5">
                        <span className="text-[10px] font-bold uppercase text-muted-foreground">Saldo Atual</span>
                        <MoneyInput 
                          value={manualCurrentBalance || 0} 
                          onValueChange={setManualCurrentBalance} 
                          placeholder="R$ 0,00"
