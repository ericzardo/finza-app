...
describe("POST /workspaces", () => {
	test("cria workspace com member OWNER e 7 categorias", async () => {
		...
		try {
			...
			const inbox = await server.prisma.bucket.findFirst({
				where: { workspace_id: parsed.data.id, is_default: true },
			});
...
describe("GET /workspaces/:workspaceId/summary", () => {
	...
	test("exclui distribuições internas do patrimônio global, mas as inclui na alocação por bucket", async () => {
		const server = await setupTestServer();

		try {
			const user = await createTestUser(server);
			const workspace = await createTestWorkspace(server, user.id);
			const inboxBucket = await server.prisma.bucket.create({
				data: {
					workspace_id: workspace.id,
			...
			await server.prisma.transaction.createMany({
				data: [
					...
					},
					{
						...
						type: TransactionType.EXPENSE,
						amount: 300,
						description: "Distribuição do inbox",
					...
					},
					{
						...
						type: TransactionType.INCOME,
						amount: 300,
						description: "Distribuição recebida",
						date: new Date("2024-01-11"),
						is_paid: true,
						internal_type: "DISTRIBUTION",
					},
			...
			const body = response.json();
			expect(body.currentBalance).toBe(1050);
			expect(body.totalBalance).toBe(1050);
			expect(body.distribution).toHaveLength(2);

			const inbox = body.distribution.find(
				(item: { bucketId: string }) => item.bucketId === inboxBucket.id,
			);
			expect(inbox.amount).toBe(700);

			const lazer = body.distribution.find(
				(item: { bucketId: string }) => item.bucketId === bucket.id,
			);
	...
	test("ignora startDate e endDate e retorna o summary all-time", async () => {
		const server = await setupTestServer();
		...
		try {
			...
			const response = await server.inject({
				method: "GET",
				url: `/workspaces/${workspace.id}/summary?startDate=2024-01-01&endDate=2024-01-31`,
				cookies: buildAuthCookie(user),
				headers: { "x-workspace-id": workspace.id },
			});
			...
			const body = response.json();
			expect(body.currentBalance).toBe(1037.4);
			expect(body.totalBalance).toBe(1037.4);

			const inbox = body.distribution.find(
				(item: { bucketId: string }) => item.bucketId === inboxBucket.id,
			);
			expect(inbox.amount).toBeCloseTo(737.4, 2);

			const target = body.distribution.find(
				(item: { bucketId: string }) => item.bucketId === bucket.id,
			);
