...
export async function createDistribution(
  ...
  // Calcular saldo disponível da transação e blindar pelo saldo real do inbox
  ...
  const finalAvailable = Math.max(
    0,
    Math.min(currentAvailable, currentInboxBalance),
  );
  const requestedTotal = distributions.reduce((sum, d) => sum + d.amount, 0);

  if (requestedTotal > finalAvailable) {
    throw new AppError(
      ErrorCode.BAD_REQUEST,
      400,
      `Saldo insuficiente para distribuição. Disponível: ${finalAvailable}, solicitado: ${requestedTotal}`,
    );
  }
  ...
  const newAvailable = finalAvailable - requestedTotal;
